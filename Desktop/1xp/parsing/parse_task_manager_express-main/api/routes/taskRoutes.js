...
const { addRecurringTask } = require("../../services/queueService");
...
router.post("/parser", async (req, res) => {
  ...
  try {
    ...
    // Добавляем задачу как повторяющуюся
    ...
    res.status(200).json({
      message: "Задача успешно создана и запланирована как повторяющаяся.",
      task: newTask,
    });
  } catch (error) {
    console.error("Ошибка при создании задачи:", error.message);
